...
class Obstacle():
        ...
        def move(self):
                ...
                while not rospy.is_shutdown():
                        ...
                        print(farthest_obstacle_direction)

                        self.error = self.section['front']-self.threshold

                        if abs(self.error) > 0.3: 
                            # print('front')
                            vel_msg.linear.x = self.Kp_linear/(self.error+2)
                            vel_msg.angular.z= 0
                            velocity_publisher.publish(vel_msg)
                            
                        elif farthest_obstacle_direction[0]=='left':
                                    # print('left')
                                    vel_msg.linear.x = self.Kp_linear/(self.error+2)
                                    vel_msg.angular.z = self.Kp_angular*self.error*2
                                    velocity_publisher.publish(vel_msg)
                        elif farthest_obstacle_direction[0]=='right':
                                    # print('right')
                                    vel_msg.linear.x = self.Kp_linear/(self.error+2)
                                    vel_msg.angular.z = -self.Kp_angular*self.error*2
                                    velocity_publisher.publish(vel_msg)
